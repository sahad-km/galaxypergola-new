...
import { Shield, ArrowRight, MagnifyingGlass, Sparkle } from '@phosphor-icons/react';
...
export default function ProductsCatalogClient() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-12 bg-white p-4 sm:p-6 rounded-3xl shadow-sm border border-neutral-100">
        ...
        <div className="flex flex-wrap gap-2 justify-center md:justify-start w-full md:w-auto">
          {categories.map((cat) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-all duration-300 relative ${
                activeCategory === cat.id
                  ? 'text-white'
                  : 'bg-neutral-50 hover:bg-neutral-100 text-neutral-charcoal border border-neutral-200'
              }`}
            >
              {activeCategory === cat.id && (
                <motion.span
                  layoutId="catalogActiveCategoryBg"
        ...
        <div className="relative w-full md:w-72 flex-shrink-0">
          ...
          <input
            ...
            className="w-full pl-11 pr-4 py-2.5 bg-neutral-50 text-neutral-charcoal text-xs font-semibold rounded-full border border-neutral-200 focus:outline-none focus:border-primary transition-all"
          />
        </div>
